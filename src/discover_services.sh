...
update_socat_rules() {
  ...
  if [ -z "$socat_pid" ]; then
    echo "[OVPN-GATEWAY] Forwarding ${forward_protocol} port $forward_port to container $container_id ($container_ip)"
...
process_container() {
  ...
  forward_configs=$(echo $container_labels | jq -r '.["openvpn_gateway.forward"] // empty')
  auto_forward_enabled=$(echo $container_labels | jq -r '.["openvpn_gateway.auto"] // "false"')
...
for container_id in $(docker ps -q -f label=openvpn_gateway.auto=true; docker ps -q -f label=openvpn_gateway.forward); do
  process_container $container_id
done

# Listen for Docker events
(
  docker events --filter label=openvpn_gateway.auto=true --format '{{json .}}' &
  docker events --filter label=openvpn_gateway.forward --format '{{json .}}' &
  wait
) | while read line; do
  ...
  case "$event_action" in
      "create")
        echo "[OVPN-GATEWAY] Container $container_id created."
        process_container $container_id
        ;;
      "start")
        echo "[OVPN-GATEWAY] Container $container_id started."
        process_container $container_id
        ;;
      "stop")
        echo "[OVPN-GATEWAY] Container $container_id stopped."
        for forward_port in $(pgrep -f "socat.*CONNECT:${container_id}"); do
          socat_pid=$(pgrep -f "socat.*CONNECT:.*:${forward_port}")
          echo "[OVPN-GATEWAY] Stopping socat process for port $forward_port (PID: $socat_pid)"
          kill $socat_pid
        done
        ;;
      "destroy")
