...
if [ ! -f "$OPENVPN_CONFIG_FILE" ]; then
  echo "Error: OpenVPN config file not found: $OPENVPN_CONFIG_FILE"
  exit 1
fi
...
# Start OpenVPN
echo "[VPN-GATEWAY] Starting OpenVPN with config $OPENVPN_CONFIG_FILE"
...
while ! pgrep -x "openvpn" > /dev/null; do
  ...
  if [ $counter -gt 30 ]; then
    echo "[VPN-GATEWAY] Error: OpenVPN failed to start"
    exit 1
  fi
  sleep 1
done
